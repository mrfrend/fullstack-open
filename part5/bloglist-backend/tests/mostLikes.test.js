...
describe('most likes', () => {
	...
	test('of one author equals sum of likes from theirs posts', () => {
		...
		const result = mostLikes(blogs)
		assert.deepStrictEqual(result, { author: 'Ivan Deshchenko', likes: 6 })
	})
	...
	test('of different authors calculated right', () => {
		...
		const result = mostLikes(blogs)
		assert.deepStrictEqual(result, { author: 'Peter Deshchenko', likes: 46 })
	})
})